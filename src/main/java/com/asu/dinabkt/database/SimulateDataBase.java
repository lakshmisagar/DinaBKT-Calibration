...
public class SimulateDataBase {
	public static Double fillrandom_initalMastery[];
	public static Double fillrandom_Learn[];
	public static Double fillrandom_slip[];
	public static Double fillrandom_guess[];
	static SessionFactory sf_OPE_Class_25;
	static Session session25;
	static boolean isSimulatedRandom = true;
	...
	private static void setSQMap() {
		...
		for (int st = 0; st < GlobalConstants.total_Students; st++) {
			...
			for (Integer Q : generated) {
				...
				int A = count;
				/*
				 * if(A==1)correct=1; if(A==2)correct=0; if(A==3)correct=0;
				 * if(A==4)correct=1;
				 */
				...
				// System.out.println("getQuestion ("+st+","+A+")
				// "+Utils.getQuestion(st, A));
				// System.out.println(st+" "+A+" "+Q+" = "+correct);
				// System.out.println("1 SAGAR
				// getAnswer_S_A_Q("+st+","+A+","+Utils.getQuestion(st,
				// A)+")"+Utils.getAnswer(st, A));
			}
	...
	private static void generateMessagesToSeatr() {
		...
		/*
		 * create table seatr_message( `id` int(11) unsigned NOT NULL
		 * AUTO_INCREMENT, `student_id` int(11) unsigned NOT NULL, `question_id`
		 * int(11) unsigned DEFAULT NULL, `format_id` int(11) unsigned DEFAULT
		 * NULL, `correct` tinyint(4) NOT NULL, `timestamp` VARCHAR(100) NOT
		 * NULL, PRIMARY KEY (`id`) );
		 */
		...
		for (int A = 0; A < GlobalConstants.total_attempts_per_student; A++) {
			for (int St = 0; St < GlobalConstants.total_Students; St++) {
				...
				for (int list_K = 0; list_K < KCs.size(); list_K++) {
					...
					double learn_mul_one_minus_comp = Operations.multiplyDouble(Utils.getLearnMap(list_K),
							one_minus_comp);
					double CompetenceValue = Operations.addDouble(Utils.getCompetence(S, list_K),
							learn_mul_one_minus_comp);
